...
test('project manifest is the vendor-neutral GroundAtlas control file', () => {
	...
	assert.equal(manifest.project.id, 'reify')
	assert.equal(manifest.project.repository, 'https://github.com/SylphxAI/reify')
...
test('Doctrine adapter remains Sylphx-specific and package proof stays package-owned', () => {
	...
	assert.equal(doctrine.project.repo, 'SylphxAI/reify')
...
test('root validation follows the current CI baseline', () => {
	...
	assert.equal(manifest.scripts.validate, 'bun run lint && bun run typecheck && bun test && bun run build')
